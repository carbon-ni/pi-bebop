...
Even after a user knows a Crew and Member name, current commands require choosing a source Pi session and can fail when that source contacts itself. Bebop must resolve transport internally and handle ambiguity deterministically.
...
- [ ] Exact Crew selector and exact Member name route deterministically across zero, one, or multiple live local candidates.
...
- [ ] A Member hosted by the selected source session can be contacted without the current self-target failure; resolution uses a safe local route or another eligible Crew Member according to the contract.
...
- [ ] Authorization, Crew membership, Guest capability boundaries, and action-specific permissions are checked before transport.
