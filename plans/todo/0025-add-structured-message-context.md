...
4. Make TASK-0031's crew-aware follow-up/immediate tools derive `{ name, role }` from active membership. Keep origin independent from optional reply routing so all wait/reply modes preserve attribution.
...
- [ ] `send_follow_up`, `send_immediate`, and joined `send_to_session` derive origin at execute time from current membership—not session name, target, caller args, or stale cached identity; leave/rejoin changes attribution and unjoined behavior is explicit.
...
- [ ] `send_follow_up`, `send_immediate`, direct session/socket messaging, and `pi-bebop send` expose compatible instruction/origin semantics; CLI repeatable instruction flags preserve order and optional `--from` is explicitly unverified.
