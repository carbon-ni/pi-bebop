...
Implement approved TASK-0079 contract end to end. Current yielding-wait code
parks waits but does not suspend `pi-auto`, so `/auto "wait or continue"`
repeats same wait before terminal event. Current Request state also closes at
idle instead of giving responder one reminder and bounded post-idle Response
window.
...
- [ ] `wait_for_member_idle` remains independent and returns/yields only already-idle, became-idle, offline, or its own timeout; existing CLI blocking contract remains compatible.
...
- [ ] Two-runtime integration covers mutual idle waits, mutual nested Member requests, target already idle before request, Response before idle, Response after reminder, grace timeout, hard timeout, offline, busy-run buffering, and auto loop preservation without sleeps.
...
- [ ] Tool/result wording says parked/suspended rather than implying completed yield, and lead loop `wait or continue` remains valid without repeated wait transcript.
