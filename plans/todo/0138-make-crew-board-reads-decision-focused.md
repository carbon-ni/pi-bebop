...
- [ ] A red test proves current `JSON.stringify(result)` output fails the exact one-Post compact-text contract.
- [ ] One Post content is exactly `#<sequence> [<kind>] <name> (<role>)\n<message>`; message bytes and attribution remain unchanged.
- [ ] Empty and multi-Post pages are deterministic, compact, and preserve result order with no synthetic ranking or interpretation.
- [ ] References/link, continuation cursor/guidance, and corruption/quarantine/truncation warnings appear only when actionable; absent/empty/false/zero values emit no placeholder text.
- [ ] Default content omits `version`, `id`, `createdAt`, `semanticFingerprint`, `redactions`, `nextCursor:null`, `hasMore:false`, and zero diagnostic fields.
- [ ] Structured details use one bounded public projection, retain exact `post_id` only for linking, and omit all internal/default metadata above; content and details do not each duplicate the canonical store result.
- [ ] Error paths keep the existing canonical Actionable Error envelope and do not expose store exceptions or paths.
- [ ] `leave_crew_post`, `/crew board`, application/domain/store persistence, cursors, filtering, ordering, and read-only semantics remain unchanged.
- [ ] Representative measurement records exact UTF-8 bytes for current JSON, raw TOON, and compact text without claiming universal savings.
- [ ] Focused tool tests, format/type/lint/architecture/package checks, and fresh exact-hash watcher gate pass.
