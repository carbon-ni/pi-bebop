...
- [ ] Accepted is never presented as answered; `docs/CREW-INTERACTION.md` outcome codes remain exact and identical across formats, including discovery timeout, delivery acceptance-unknown, response-after-idle timeout, total timeout, cancellation, malformed Response, route conflict, and route loss.
...
- [ ] Retrying after an uncertain transport result does not silently create duplicate correlated requests; delivery timeout/SIGINT reports `acceptance: unknown` and `safeRetry: false`, while discovery timeout proves no delivery began.
