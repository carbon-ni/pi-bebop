...
- [ ] `pi-bebop ask <crew-selector[/member]> <question>` resolves the product target, submits exactly one Member request, waits for exactly its correlated Response, and returns in one CLI invocation.
...
- [ ] Default and override timeouts use one documented duration grammar, state their bounds in help, and never permit an unbounded wait.
