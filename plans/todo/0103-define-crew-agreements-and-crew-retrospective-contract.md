...
- [ ] Contract makes visible Crew work Crew-readable by default while separating security redaction from privacy: credentials/secrets are removed, hidden model reasoning is unavailable, and visible messages/tool results/artifacts may contribute evidence.
- [ ] Evidence, interpretation, and Agreement proposal remain distinct; collected evidence and a Member report never automatically become Current Crew Agreements.
- [ ] Contract resolves duplicate start, unavailable facilitator, takeover, missing/late Responses, objection/correction, stale base revision, interval boundaries, evidence arriving after freeze, clock rollback, and restart semantics.
- [ ] Product contract states what is deliberately deferred: automatic start, Crew-consensus activation, hot reload, registry, semantic conflict resolution, and unbounded raw transcript dumps.
