...
| Wait for the oldest terminal outbound Request outcome                                                                                                    | `wait_for_request_outcome`  | Requester-side: call alone after sending a Member request; ends the current run and resumes in a later terminal outcome turn; no polling, inbound handling, or unrelated activity. |
| Block this run until another member's Pi is mechanically idle, goes offline, the bounded timeout expires, or an accepted Bebop message releases the wait | `wait_for_member_idle`      | One-shot bounded blocking wait; message-received never implies reply, idle, task completion, or availability. Request-outcome waiting ends the current run.                                           |
