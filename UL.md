...
| **Crew Idle Gate**              | One-shot bounded coordination primitive over a frozen manifest-order scope: every other Member by omission or explicit exact Members; scoped ready requires one Final Crew Status Round to observe every target online/idle and never grants Lead authority.                   | wait-all completion, scheduler, atomic Crew snapshot          |
...
| **Crew Idle Lock**              | Whole-Crew agent condition where a blocking Crew Idle Gate covers every other frozen Member and each target explicitly owns a Blocking-wait marker. It releases only that agent caller with `wait-lock`; proper subsets and slash observation cannot prove it.                 | arbitrary deadlock proof, remote cancellation, recovery       |
...
| **Requester**                   | Transient per-request role: the member who sent a Member request and alone yields for its next Request event with wait_for_request_outcome. It may receive one 180s still-pending reminder, then send an ordinary Follow-up or wait again. Not a Crew role or authority.       | lead by default, owner, asker with polling                    |
...
| **Request outcome**             | Oldest outbound Request event: terminal Response/offline/timeout or one nonterminal still-pending reminder at 180s. Idle itself is NOT an outcome. A settled queue returns all-settled; events are not progress, task state, or completion proof.                              | monitoring, status, completion proof                          |
...
| **Crew Message Log**            | One bounded project-local append-only evidence source for allow-listed Bebop Crew messaging lifecycle facts and deterministically redacted visible payload representation; every Current Member can explicitly read all retained active-layout history.                        | Inbox, Crew Board, transcript, monitoring feed                |
| **Log Entry**                   | One immutable canonical Crew Message Log record for an allow-listed mechanical message event, capture boundary, explicit gap, retention omission, or frozen coverage snapshot; absence never proves no activity.                                                               | message, audit truth, read receipt, completion proof          |
| **Messaging Review**            | Explicit fixed-interval learning record combining immutable Crew Message Log evidence/gaps with authenticated attributed Member feedback while separating evidence, interpretation, proposal, and activation.                                                                  | analytics dashboard, preference inference, performance review |
...
| **Inbox**                    | Durable per-member message queue accepted while recipient may be offline; a best-effort hint triggers an offer only at the recipient-owned authoritative idle boundary as one normal follow-up.                                                                                                            | task board, workflow engine, mailbox UI                    |
...
| **Crew Correspondence**      | One durable, same-machine letter from a Current Member to another Crew's configured Intake contact by explicit absolute manifest path. It is persisted only and each ask/reply is a separate one-way turn.                                                                                                 | live chat, authenticated request, thread, delivery receipt |
| **Crew Return Address**      | Structured claimed source Crew manifest path, with optional display label, derived from joined Membership and carried by Crew Correspondence only as the explicit target for a later reply. It is not a live route, callback, discovery handle, authentication proof, or promise the path remains current. | session ID, alias, socket, replyTo, credential             |
...
| **Compaction Delivery Gate** | Receiver-owned boundary that durably defers model-bound Bebop coordination during Pi compaction and hands it to Pi after the safe lifecycle boundary. It never grants the sender compaction visibility or control.                                                                                         | sender retry loop, scheduler, pause control                |
| **Ambiguous handoff**        | Crash window after the gate records `handing-off` but before public Pi 0.84.3 evidence can prove durable model handoff. Evidence absence permits one possible-duplicate replay with the same Delivery ID; it never proves the first handoff did not occur.                                                 | failed delivery, exactly-once proof                        |
| **Delivery replay**          | One at-least-once recovery handoff for an Ambiguous handoff. It reuses the immutable message and Delivery ID, adds explicit possible-duplicate provenance, and is not a new acceptance or new message.                                                                                                     | retry by sender, duplicate request                         |
| **Delivery ID**              | Opaque stable receiver-assigned identifier for one persisted coordination envelope and its possible replay. It supports evidence and deduplication but is not model-visible attribution, routing, authentication, or task identity.                                                                        | Request ID, message content, credential                    |
...
| Leave durable message for online or offline peer                                                                                                         | `send_to_inbox`             | Durable per-member queue; persists if recipient is offline; best-effort hint and idle-boundary Follow-up are separate from persistence.                                                                 |
| Durable fan-out to every other member                                                                                                                    | `broadcast_to_crew`         | One non-interrupting message persisted to every other member, then best-effort hinted and handed off at each recipient's idle boundary as a normal follow-up.                                           |
| Exchange one durable letter with another local Crew                                                                                                      | `send_to_crew`              | Joined-only explicit-path correspondence; it persists for the target contact and carries claimed Origin/Return Address, never a live reply route or Response.                                           |
| Append durable shared project context without delivery                                                                                                   | `leave_crew_post`           | Adds one bounded attributed tip, kudos, feedback, warning, or note to the shared pull-based Crew Board; it does not notify anyone.                                                                      |
| Pull bounded shared project context                                                                                                                      | `read_crew_board`           | Reads a deterministic Board page without consuming Posts, creating read state, or claiming another Member saw them.                                                                                     |
...
| Wait for the oldest outbound Request event                                                                                                               | `wait_for_request_outcome`  | Requester-side yielding loop: one Response/offline/timeout or one nonterminal still-pending reminder at 180s; no polling/inbound handling; settled requests return `all-settled`.                       |
...
| Block this run until a final round observes all omitted/explicit frozen Crew targets idle, or a bounded non-ready outcome releases it                    | `wait_for_crew_idle`        | Event-driven gate with optional exact `members`; selected ready is scoped, and only full-roster agent coverage may return caller-local `wait-lock`. Neither proves completion or grants Lead authority. |
...
Current Member sends Crew Correspondence only through explicit target Crew manifest path
Crew Correspondence derives claimed Origin and Crew Return Address from source Membership
Crew Correspondence persists for target Crew contact and may be replied to only by another explicit one-way Crew Correspondence
Crew Return Address is a claimed path, never a live route, callback, authentication proof, or Response promise
...
Crew Membership gives every Current Member identical Crew Board read-and-append capability
Current Member appends immutable attributed Crew Post to one shared active-layout Crew Board
Crew Board read is explicit and non-consuming; it never delivers or records readership
Crew Post kind and links add browsing/context only, never truth, rating, workflow, or authority
Allow-listed Crew messaging transitions may append immutable bounded Log Entries after their mechanical outcome
Crew Message Log capture never changes delivery, and a capture failure becomes an explicit durable/volatile evidence gap when observable
Every Current Member can pull all retained active-layout Log history, including history predating join/rejoin; Role never changes visibility
Crew Message Log keeps Membership-derived identity separate from claimed Origin and excludes callbacks, sockets, hidden reasoning, and raw errors
TASK-0130 freezes pre-review endpoint coverage before TASK-0131 read-only inspection; readers cannot checkpoint, append, retain, or repair
Messaging Review combines fixed-interval Log evidence with direct attributed Member feedback; usage and silence never prove preference
...
Request event is the oldest outbound terminal outcome or one still-pending reminder; all-settled is normal at zero, not unrelated Crew activity
...
Crew Idle Gate omission freezes every other manifest Member; explicit selection freezes exact names in manifest order
Crew Idle Gate result labels request scope all/selected and whether frozen targets cover every other Member
Crew Idle Gate ready requires one Final Crew Status Round to observe every frozen target online/idle in stated scope
Selected ready never claims an unselected Member or whole Crew is ready
Human /crew member-idle shares target/round/deadline rules but never blocks an agent run or owns a Blocking-wait marker
...
- **Crew Board/messaging:** Crew Board is shared pull-based retained context with no recipient or delivery. Follow-up, Redirect, Inbox, Member request, and Crew Broadcast all have delivery semantics; none is a Board alias.
- **Crew Board/access:** Membership is the only Bebop capability boundary: every Current Member can read and append. There are no private Posts, Role permissions, owners, moderators, or tiers; this is not an operating-system filesystem-security claim.
- **Crew Board/discovery:** joined context and help teach the Board affordance only. They never read or inject Crew Post content, counts, references, or cursors; a Member explicitly calls the read operation.
- **Crew Post/kind/authority:** tip, kudos, feedback, warning, and note are browsing labels for fallible attributed statements. They are never ratings, reputation, sentiment, productivity/performance evidence, tasks, instructions, votes, Agreements, or authority.
- **Crew Message Log/Inbox/Board:** Inbox persists per-recipient delivery; Crew Board retains caller-authored shared Posts; Crew Message Log internally captures allow-listed mechanical messaging evidence. Log inspection never sends, hands off, appends a Board Post, or records readership.
- **Crew Message Log/access/history:** Current Membership grants all retained active-layout history, including pre-join/rejoin Entries. This is deliberate Crew transparency, not capture-time consent/ACL or an operating-system filesystem guarantee; leaving removes application access without rewriting retained attribution.
- **Crew Message Log/capture truth:** delivery outcome and evidence outcome are separate. A missing Entry, unclean epoch, unavailable endpoint, corruption, or pruned interval is an explicit gap and never proof that no message or work existed.
- **Log Entry/Origin:** source/target Member fields come from trusted application Membership/manifest snapshots. Origin is a separately retained claimed attribution and cannot authenticate, override identity, or grant authority.
- **Messaging Review/preference:** mechanical frequency, non-use, failures, silence, timeout, and offline outcomes never reveal preference, satisfaction, intent, agreement, productivity, completion, or Member quality. Only direct authenticated attributed feedback states a Member view, and disagreement remains visible.
- **Coverage collection/read:** TASK-0130's explicit bounded pre-review operation owns endpoint checkpoints, gap writes, and immutable snapshot freeze. TASK-0131 consumes snapshot ID/hash through reader-only dependencies and cannot contact endpoints or mutate Log/Inbox/retention state.
- **Crew Post/correction:** Posts are immutable. Same-author supersedes and attributed disputes append backward links without rewriting prior bytes or silently deciding truth/consensus.
...
- **Crew Idle Gate/Member Idle Wait:** Member Idle Wait targets one exact Member and returns from that Member's terminal. Crew Idle Gate freezes every other Member by omission or a bounded explicit exact-name scope, waits event-by-event, and requires a new full frozen-scope round before ready.
...
- **Crew Idle Lock/busy:** only a blocking agent gate with exact full-roster coverage plus explicit transient target markers proves the whole-Crew lock predicate. Generic busy/compacting Activity, proper subsets, slash observation, task state, and conversation never do.
...
- **Request outcome/activity:** Request outcome wait yields for the oldest outbound terminal outcome or one fixed 180-second still-pending reminder; it returns all-settled at zero and never monitors unrelated Crew activity.
