...
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- |
| **Crew**                    | Trusted, project-local set of configured Pi members that may be independently online or offline. A Guest-enabled Crew also has a stable public identity and display name.                                                                                                                     | team, cluster, pool                                     |
| **Crew manifest**           | Authoritative file that defines crew identity, display name, members, roles, endpoints, instructions, Guest approvers, and presence policy.                                                                                                                                                   | config, registry                                        |
| **Member**                  | Pi session that claims one identity configured by crew manifest. Members are distinct from externally admitted Guests.                                                                                                                                                                        | peer, agent, session when discussing crew identity      |
| **Guest**                   | External Pi session with one stable identity explicitly admitted as a limited messaging participant in one or more exact Crews; it never becomes a configured Member or receives a Crew role.                                                                                                 | external member, visitor, peer                          |
...
| **Crew directory**          | Bounded local view mapping exact Crew Selectors to trusted eligible Crew Locators, availability, and freshness without exposing session or endpoint transport.                                                                                                                                | `pi-bebop session live`, filesystem scan                |
| **Pi Session**              | Pi-owned persisted JSONL conversation with an exact full session ID, persisted file, working directory, and SessionManager-reported root. Bebop treats its conversation body as opaque.                                                                                                  | Crew Session, Member identity, transcript               |
| **Crew Session**            | Named, machine-local record linking one exact Crew to captured configured Members and their exact Pi Sessions. It is a bookmark/snapshot, not a shared conversation, process group, workflow, task, lock, or liveness claim. | shared conversation, latest session, workflow          |
| **Member Session link**     | Immutable-at-capture binding of Crew identity, exact configured Member name/role, and exact persisted Pi Session identity plus safe reopen metadata.                                                                                                                                           | guessed session, role-to-session inference              |
| **Crew Session capture**    | Explicit snapshot of currently joined, online Members belonging to one exact trusted Crew; valid links and bounded missing reasons are retained in manifest order.                                                                                                                           | shutdown hook, history scan, whole-Crew launcher        |
| **Member Session addition** | Explicit capture of one previously missing exact Member into an existing partial Crew Session without changing other links.                                                                                                                                                                  | replacement, implicit recapture                         |
| **Member Session resolution** | Read-only validation of one stored link that returns evidence and a manual Pi startup specification; it never starts Pi or claims a lock.                                                                                                                                                | launcher, lock, automatic resume                        |
...
| **Guest capability**        | Runtime-held opaque credential authorizing one approved Guest membership; it is never model-visible, transferable, or a substitute for Crew or Guest identity.                                                                                                                                | token in prompt, role, socket path                      |
...
| **Manifest fingerprint**    | Versioned digest of canonical, schema-validated Crew identity and Member routing metadata observed at capture; it detects drift without storing manifest content.                                                                                                                           | credential, authentication proof                        |
...
| **Common instructions**     | Stable Markdown guidance selected by a version 2 crew manifest and loaded for every member before role instructions.                                                                                                                                                                          | AGENTS.md, role instructions                            |
...
| **Crew Template**           | Versioned reusable manifest, shared instructions, Role instructions, and usage guidance. It may contain deterministic project-relative configured Member endpoint paths, but no resolved runtime sockets, endpoint claims, Inbox records, sessions, credentials, or implicit model selection. | live Crew, runner configuration                         |
| **Crew Configuration**      | A Crew Template plus explicit runner-selected models, providers, thinking levels, case inputs, bounds, repetitions, and environment. It belongs to the external evaluation harness.                                                                                                           | Crew manifest, implicit model choice                    |
| **Crew Eval Case**          | One fixed prompt, optional disposable fixtures, expected outcome, and assertions declared before outputs are inspected.                                                                                                                                                                       | live task, message, hidden test                         |
| **Crew Eval Run**           | One isolated execution of one Crew Configuration against one Crew Eval Case.                                                                                                                                                                                                                  | benchmark aggregate, live session                       |
| **Crew Deliverable**        | Bounded artifact selected by a Template for grading; in the Council Template, the terminal deliverable is the Chair's final verdict.                                                                                                                                                          | vote, transcript, proof of truth                        |
| **Crew Baseline**           | Explicit comparison Configuration, such as a single agent or prior Template snapshot; an unlabeled absence of Crew is not a baseline.                                                                                                                                                         | control by omission, no-Crew run                        |
| **Council**                 | Default exactly-three-member evaluation arrangement: one Chair and exactly two independent Judges. Extra reviewers or a Worker are explicit optional extensions.                                                                                                                              | committee, inferred model diversity                     |
| **Chair**                   | Council member that frames the request and rubric, requests independent judgments, exposes agreement/disagreement, and reports the terminal verdict.                                                                                                                                          | manager, authority, proof                               |
| **Judge**                   | Council member that independently evaluates supplied evidence and artifact against the same rubric, returning verdict, evidence, uncertainty, and missing information before seeing peer judgment.                                                                                            | voter, authority, proof                                 |
| **Worker**                  | Explicit optional artifact producer. A Council accepts an externally supplied artifact and does not silently include a Worker.                                                                                                                                                                | implicit producer, required Council member              |
| **Evaluation harness**      | External software that owns process lifecycle, case orchestration, isolation, assertions, grading, aggregation, and reports. Bebop remains transport for identity, membership, messaging, and truthful outcomes.                                                                              | Bebop workflow engine, hidden runner                    |
...
| **External actor**          | Local process or Pi session that sends a crew message without joined Member or Guest identity.                                                                                                                                                                                                | crew member, authenticated caller                       |
...
| **Crew Principal**          | External person, service, or Pi process explicitly configured as one Crew's outbound contact. Principal names a product recipient, not a permission, Crew role, or command authority.                                                                                                         | coordinator as authority, external member, Guest        |
| **Principal identity**      | Stable, case-sensitive manifest-authored identity expected at the Principal contact; it selects and attributes the destination but does not authenticate every Principal-originated message.                                                                                                  | display name, session alias, credential                 |
| **Principal contact**       | Explicit Crew policy that permits bounded one-way outbound messages to one Principal identity. It is distinct from Crew contact, which triages external Crew Intake.                                                                                                                          | reverse Intake, callback permission, inbox              |
| **Principal route binding** | Application-owned binding between one Principal identity and one approved destination. Opaque route material stays outside the manifest and normal output; binding authorizes receipt only.                                                                                                   | Member endpoint, Runtime socket, Guest capability       |
| **Principal message**       | One-way outbound message attributed to the sending Crew and permitted Member; it expects no Response and never proves that the Principal read, approved, or acted on it.                                                                                                                      | Member request, task, acknowledgement                   |
| **Crew Broadcast**          | Internal transient fan-out initiated by a current joined Member or approved Guest; one normal Broadcast Follow-up is attempted for every other permitted participant, while durable delivery remains explicit for Members only.                                                               | intake, shared inbox, redirect-all, team broadcast      |
...
| **Follow-up**                | Normal non-interrupting member message delivered after target finishes active work when target is busy; Broadcast uses this delivery behavior.                                                                                  | deferred, inbox, non-urgent                           |
...
| **Principal delivery state** | Observable product state for one Principal message: persisted, pending, handed-to-Principal, rejected, expired, revoked, unknown, or cancelled. It never implies a Response or task completion.                                 | delivered, read, completed                            |
...
| **Disposition**     | Acknowledged delivery outcome: direct, queued, or redirected; Broadcast reports each recipient as delivered or failed.                            | status without qualification             |
...
| Transient fan-out to every other member                                                                                                                  | `broadcast_to_crew`         | One non-interrupting Broadcast Follow-up attempted for every other member; failures are explicit and `send_to_inbox` remains the durable option.                                      |
...
| Wait for the oldest terminal outbound Request outcome                                                                                                    | `wait_for_request_outcome`  | Requester-side: blocks the current tool call after sending a Member request; an accepted inbound message releases it without settling the request; one waiter, cancellation, and bounded outcomes. |
...
Crew Session links one exact Crew to exact captured Pi Sessions by configured Member
Crew Session capture observes currently joined online Members and retains bounded missing reasons
Member Session addition fills one missing Member link without changing other links
Member Session resolution validates one stored link and returns a manual Pi startup specification; it never starts Pi
Pi Session remains Pi-owned persisted conversation state; Bebop does not parse its body
...
Guest is reached through its approved callback endpoint
...
Crew manifest explicitly lists Guest approver Member names
Crew manifest may explicitly configure one Principal contact, identity, route-binding method, allowed Member senders, and bounded delivery policy
Crew Template defines reusable Crew structure and guidance without live runtime state
Crew Configuration adds explicit runner model/provider/thinking choices, Cases, bounds, repetitions, and environment to one Crew Template
Crew Eval Case fixes prompt, fixtures, expected outcome, and predeclared assertions
Crew Eval Run isolates one Configuration against one Case
Crew Deliverable is the bounded artifact selected for grading; Council accepts it externally
Crew Baseline is an explicit comparable Configuration, never an unlabeled absence of Crew
Council has exactly one Chair and exactly two independent Judges by default
Evaluation harness owns lifecycle, orchestration, grading, and reports; Bebop remains transport for identity, membership, messaging, and truthful outcomes
Principal contact is outbound-only from authorized Crew Members to one configured Principal identity
Principal route binding authorizes receipt only and never grants membership, Guest capability, inbound messaging, or approval authority
Principal messages use a bounded Crew-owned outbox with stable identity, FIFO ordering, and idempotent completion evidence
Missing or empty Guest approver policy disables Guest admission
...
Crew Broadcast is initiated only by a Current joined Member or approved Guest within an exact Crew
Crew Broadcast excludes the sender by canonical participant identity within that Crew
Guest join request travels through one live Member socket but creates no membership until approval
Guest leave or Crew revoke invalidates that Crew membership only; other Guest memberships remain active
Guest Origin is labelled `(guest)` and is attribution, not trusted content or role authority
...
- **Crew Principal/Crew contact:** Principal contact is Crew → external, one-way outbound delivery. Crew contact is the exact Member who triages External crew messages from Crew Intake. Neither is inferred from lead, role, first Member, online state, or previous recipient.
- **Principal/External actor:** a Principal is a configured product recipient, while an External actor is an unverified actor sending without Member or Guest identity. A Principal route does not authenticate an External actor or grant inbound authority.
- **Principal route binding/endpoint:** route binding selects an approved destination but is not a Member endpoint, Runtime socket, Guest capability, session alias, or model-visible credential.
- **Principal message/Response:** a Principal message is one-way and expects no Response. Persisted, handed-to-Principal, and acknowledged are distinct; none proves reading, approval, action, or completion.
- **Principal contact/Intake/Inbox:** Crew Intake is External actor → configured Crew contact; Principal contact is Crew → external Principal. Inbox is a durable per-Member queue; a Principal outbox is separate even when both use bounded durable storage.
- **Principal/Ask:** Ask requires an authorized current Member or approved Guest and yields one correlated Response or Ask outcome. Principal contact has no Request/Response semantics and cannot be used to invoke Ask.
...
- **Crew Session/Pi Session:** A Crew Session is a local bookmark of exact Pi Sessions, not a shared conversation, process group, task, lock, or liveness claim. Pi owns session lifecycle and conversation semantics.
- **Capture/addition:** Capture is explicit and snapshots currently joined online Members; addition fills one missing link only. Neither guesses a recent session, replaces an existing binding, or silently omits a configured Member.
- **Resolution/lock:** Member Session resolution is a bounded observation before a user manually runs `pi --session <absolute-file>`. It cannot lock a JSONL or guarantee another process will not open it first.
- **Session references/privacy:** Full Pi Session IDs, files, cwd, and roots appear only on explicit Crew Session commands or safe diagnostics, never in normal Crew/member/status/list output.
...
- **Request outcome/activity:** Request outcome wait returns the oldest terminal outbound Member request outcome, or a neutral accepted-message wake; it never monitors or returns unrelated Crew activity.
...
- **Request outcome waiting:** The Requester calls `wait_for_request_outcome` after sending a request; the same tool call blocks until Response, offline, bounded timeout, or an accepted inbound message. Message wake does not settle the outbound request; offline suggests reassignment or `send_to_inbox`; timeout suggests status review, reassignment, durable Inbox delivery, or urgent Redirect. `/auto` scheduling is outside Bebop's scope.
...
- **Principal outbound/core transport:** Principal contact uses a Pi Bebop application/infra seam. It must not shell out to `pi`, require pi-intray, or expose `--control-session`, `--send-session-message`, session IDs, or sockets as the product API.
