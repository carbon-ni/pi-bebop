...
export async function handleGetMessage(
	...
	const message = getLastAssistantMessage(context.ctx.sessionManager.getBranch());
	if (!message) {
		context.respond(true, "get_message", { message: null });
		return;
	}
	context.respond(true, "get_message", { message });
	return;
}
