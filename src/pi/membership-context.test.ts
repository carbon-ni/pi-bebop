...
import { appendMembershipContext, formatMembershipContext, getLatestMembershipState, MEMBERSHIP_ENTRY_TYPE, membershipStateFromRuntime } from "./membership-context.ts";
...
const membership: Membership = { manifestPath, socketPath: "/project/.pi/intray/sockets/dev.sock", globalSocketPath: "/tmp/global.sock", member: manifest.members[0], manifest };
...
test("restores latest branch-aware active or inactive membership state", () => {
	const entries = [
		...
		{ type: "custom", customType: MEMBERSHIP_ENTRY_TYPE, data: { active: true, socketPath: membership.socketPath, manifestPath } },
	];
	assert.deepEqual(getLatestMembershipState(entries), { active: true, socketPath: membership.socketPath, manifestPath });
	assert.deepEqual(membershipStateFromRuntime(membership), { active: true, socketPath: membership.socketPath, manifestPath });
});
