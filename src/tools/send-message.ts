...
export async function sendMessageToSocket(
	...
	if (wait === "turn_end") {
		...
		if (!lastMessage)
			return {
				...
				details: { turnIndex: result.turnIndex },
			};
		return {
			content: [{ type: "text", text: lastMessage.content }],
			details: { message: lastMessage, turnIndex: result.turnIndex },
		};
	}
	if (request.policy.waitUntil === "message_processed" || request.policy.waitUntil === "off") {
		return {
			content: [{ type: "text", text: `Message delivered to ${request.deliveryTarget ?? "session"}` }],
			details: result.data,
		};
	}
