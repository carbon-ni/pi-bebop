...
export type SessionTargetErrorCode =
	| "missing-target"
	| "invalid-session-id"
...
export async function resolveSessionTarget(
	...
	dependencies: SessionTargetDependencies,
	...
	if (sessionId && !isSafeSessionId(sessionId))
		throw new SessionTargetError("invalid-session-id", "Invalid session id");
	...
	if (input.socketPath?.trim()) {
		...
		try {
			...
			throw new SessionTargetError(
				"unknown-member",
				`Unknown configured crew member for socket path: ${socketPath}`,
			);
		}
		try {
		...
		if (targetSessionId || input.currentSessionId) {
			try {
				...
				if (targetSessionId && linkedSocket !== path.resolve(getSocketPath(targetSessionId))) {
					throw new SessionTargetError(
						"target-mismatch",
						"Socket path does not match the requested session target",
					);
				}
			} catch (error) {
	...
	if (!targetSessionId)
		throw new SessionTargetError("missing-target", "Missing socket path, session id, or session name");
	return {
		socketPath: getSocketPath(targetSessionId),
		sessionId: targetSessionId,
		displayTarget: sessionName ?? targetSessionId,
	};
}
