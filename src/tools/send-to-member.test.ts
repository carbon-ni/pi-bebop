...
const ok = (data: unknown = { delivered: true }) => ({
	response: { type: "response", command: "send", success: true, data },
});
...
test("sends to a manifest member with role identity, signal, and synchronous response", async () => {
	...
	const { tool } = setup(async (socketPath, command, options) => {
		...
		return { ...ok(), event: { message: { content: "done" } } };
	});
	const signal = new AbortController().signal;
	const result = await tool.execute(
		"call",
		{ member: "qa", message: "hello", mode: "follow_up" },
		signal,
		undefined,
		undefined,
	);
	...
	assert.deepEqual(calls, [
		{
			socketPath: "/project/.pi/intray/sockets/qa.sock",
			command: { type: "send", message: "hello", mode: "follow_up" },
			options: { timeout: 300000, waitForEvent: "turn_end", signal },
		},
	]);
});
...
test("rejects unjoined, unknown, and self targets without RPC", async () => {
	...
	assert.equal(
		(
			await setup(send, false).tool.execute(
				"call",
				{ member: "qa", message: "x" },
	...
	assert.match(
		(
			await setup(send).tool.execute(
				"call",
				{ member: "missing", message: "x" },
				new AbortController().signal,
				undefined,
				undefined,
			)
...
test("forwards abort signals and reports an aborted request", async () => {
	...
	const abortError = await setup(async () => {
		throw Object.assign(new Error("cancelled"), { name: "AbortError" });
	}).tool.execute("call", { member: "qa", message: "x" }, new AbortController().signal, undefined, undefined);
...
test("reuses asynchronous policy, forwards abort, and reports offline endpoints", async () => {
	...
	const { tool } = setup(async (_path, _command, options) => {
		callOptions = options;
		return ok();
	});
	const delivered = await tool.execute(
		"call",
		{ member: "qa", message: "x", wait_until: "message_processed", reply_behavior: "end_conversation" },
		controller.signal,
		undefined,
		undefined,
	);
...
test("rejects ambiguous role targets", async () => {
	...
	const result = await setup(async () => ok(), true, ambiguousMembership).tool.execute(
		"call",
		{ member: "reviewer", message: "x" },
		new AbortController().signal,
		undefined,
		undefined,
	);
