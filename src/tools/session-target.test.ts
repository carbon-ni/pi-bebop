...
test("resolves @ socket paths relative to execution cwd through the manifest", async () => {
	const target = await resolveSessionTarget(
		{ socketPath: "@.pi/intray/sockets/dev.sock", cwd: "/project", isProjectTrusted: () => true },
		deps(),
	);
	...
	await assert.rejects(
		() =>
			resolveSessionTarget(
				{
					socketPath: "/project/.pi/intray/sockets/missing.sock",
					cwd: "/project",
...
test("preserves id and alias target resolution", async () => {
	const byId = await resolveSessionTarget(
		{ sessionId: "target-id", cwd: "/project", isProjectTrusted: () => true },
		deps(),
	);
	assert.equal(byId.socketPath, getSocketPath("target-id"));
	const byName = await resolveSessionTarget(
		{ sessionName: "dev", cwd: "/project", isProjectTrusted: () => true },
		deps(),
	);
