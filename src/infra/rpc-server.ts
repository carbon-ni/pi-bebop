...
function write(socket: RpcSocket, value: Parameters<typeof serializeProtocolMessage>[0]): void { try { socket.write(serializeProtocolMessage(value)); } catch { /* Socket may be closed. */ } }
export function writeWireResponse(socket: RpcSocket, id: RpcId, method: string, result: unknown): void {
	...
	if ("code" in response) { writeWireError(socket, id, response.code, response.message, response.data); return; }
...
export function writeEvent(socket: RpcSocket, event: RpcTurnEndNotification): void {
	const notification = buildTurnEndNotification(event.subscriptionId, event.data?.message ?? null, event.data?.turnIndex);
	try { write(socket, notification); } catch { /* Socket may be closed. */ }
}
	...
	const server = net.createServer((socket) => {
		...
		socket.on("data", (chunk) => {
			...
			while (newlineIndex !== -1) {
				...
				if (parsed.error) { writeWireError(socket, null, parsed.error.code, parsed.error.message, parsed.error.data); continue; }
				const request = parsed.request!; const command = requestToCommand(request);
				if ("code" in command) { writeWireError(socket, request.id, command.code, command.message, command.data); continue; }
				void Promise.resolve(onCommand(command, socket)).catch(() => writeWireError(socket, request.id, RPC_ERROR.internal, "Internal error", { code: "internal-error" }));
			}
		});
	});
