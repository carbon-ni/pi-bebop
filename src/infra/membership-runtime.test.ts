...
test("claims an initially absent external-root configured endpoint", async () => {
	...
	const runtime = createMembershipRuntime({ loadManifest: async () => manifest, claimEndpoint: (async (socketPath: string) => { claimCalls += 1; claimedPaths.push(socketPath); return { idempotent: false }; }) as never });
