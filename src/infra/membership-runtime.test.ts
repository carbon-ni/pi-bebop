...
import { createMembershipRuntime, type Membership } from "./membership-runtime.ts";
...
describe("membership runtime", () => {
	test("surfaces the actionable manifest load cause", async () => {
		const runtime = createMembershipRuntime(
			dependencies({
				loadManifest: async () => {
					throw new Error("untrusted-path: compatibility layout rejected");
				},
			}),
		);
	...
	test("joins by authoritative socket lookup and stores normalized membership", async () => {
		...
		const result = await runtime.join({
			...joinArgs(),
			socketPath: "/project/.pi/intray/sockets/../sockets/dev.sock",
		});
	...
	test("extensionless configured endpoint joins without suffix guessing", async () => {
		let claims = 0;
		const extensionless = parseCrewManifest(
			{ version: 1, members: [{ name: "dev1", role: "developer", socket: "sockets/dev1" }] },
			"/root-B/.pi/bebop/crew.json",
		);
		const runtime = createMembershipRuntime({
			loadManifest: async () => extensionless,
			claimEndpoint: (async () => {
				claims += 1;
				return { idempotent: false };
			}) as never,
			releaseEndpoint: (async () => ({ released: true })) as never,
		});
		const result = await runtime.join({
			manifestPath: "/root-B/.pi/bebop/crew.json",
			socketPath: "/root-B/.pi/bebop/sockets/dev1",
	...
	test("dev1 versus dev1.sock fails before claim with bounded exact guidance", async () => {
		let claims = 0;
		const manifest = parseCrewManifest(
			{ version: 1, members: [{ name: "dev1", role: "developer", socket: "sockets/dev1.sock" }] },
			"/root-B/.pi/bebop/crew.json",
		);
		const runtime = createMembershipRuntime({
	...
	test("claims an initially absent external-root configured endpoint", async () => {
		...
		const manifest = parseCrewManifest(
			{ version: 1, members: [{ name: "dev1", role: "developer", socket: "sockets/dev1.sock" }] },
			"/root-B/.pi/crew/crew.json",
		);
		const runtime = createMembershipRuntime({
			loadManifest: async () => manifest,
			claimEndpoint: (async (socketPath: string) => {
				claimCalls += 1;
				claimedPaths.push(socketPath);
				return { idempotent: false };
			}) as never,
		});
		const result = await runtime.join({
		...
		assert.equal(result.ok, true);
		assert.deepEqual(claimedPaths, ["/root-B/.pi/crew/sockets/dev1.sock"]);
	...
	test("unknown member and malformed manifest fail without claiming", async () => {
		...
		const runtime = createMembershipRuntime(
			dependencies({
				loadManifest: async () => {
					throw new Error("malformed crew");
				},
				claimEndpoint: async () => {
					claims += 1;
					return { claimed: true, idempotent: false };
	...
	test("role switch claims new endpoint before releasing old", async () => {
		...
		const runtime = createMembershipRuntime(
			dependencies({
				claimEndpoint: async (endpoint) => {
					events.push(`claim:${endpoint}`);
					return { claimed: true, idempotent: false };
				},
				releaseEndpoint: async (endpoint) => {
					events.push(`release:${endpoint}`);
	...
	test("failed old release rolls back the newly claimed endpoint and preserves membership", async () => {
		...
		assert.equal(runtime.getMembership()?.member.name, "dev");
		assert.deepEqual(released, ["/project/.pi/intray/sockets/dev.sock", "/project/.pi/intray/sockets/qa.sock"]);
	});
