...
test("sends exact message without sender metadata and returns an accepted result", async () => {
	...
	const result = await sendDirectMessage({ socketPath: "/x", message: "private\ntext", mode: "steer", wait: "accepted", timeoutMs: 123 }, async (path, command, options) => {
		call = { path, command, options };
		return { response: { type: "response", command: "send", success: true, data: { delivered: true } } };
	});
	assert.deepEqual(call, { path: "/x", command: { type: "send", message: "private\ntext", mode: "steer" }, options: { timeout: 123, signal: undefined } });
...
test("rejects turn completion without an assistant response as an operational error", async () => {
	await assert.rejects(
		() => sendDirectMessage({ socketPath: "/x", message: "x", mode: "follow_up", wait: "turn_end", timeoutMs: 500, requireAssistantResponse: true }, async (_path, _command, options) => {
...
test("preserves the Pi adapter's successful no-assistant turn result", async () => {
	const result = await sendDirectMessage({ socketPath: "/x", message: "x", mode: "steer", wait: "turn_end" }, async () => ({ response: { type: "response", command: "send", success: true }, event: { turnIndex: 7 } }));
