...
test("usage error: --format=json and --format json both steer usage output", async () => {
	...
	assert.match(
		toon,
		/^pi-bebop command input failed: the command input is invalid/,
		"default usage output is human text",
	);
});
...
test("gap: text format is concise plain text, never TOON/JSON keys", async () => {
	...
	assert.equal(
		textUsage.trim(),
		"pi-bebop command input failed: the command input is invalid. Next: run the command with --help, correct the input, and retry. (code: usage)",
	);
...
test("PO: --session is not a global/root flag today; it must be added as an explicit contract decision", () => {
	...
	assert.throws(
		...
		/Invalid command '--session'; valid commands: send, crew init, crew roles, member status, member wait-idle, session list, member follow-up, member redirect, member interrupt, member inbox send, crew broadcast/,
	);
});
