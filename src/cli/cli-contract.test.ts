...
test("gap: text format is concise plain text, never TOON/JSON keys", async () => {
	...
	assert.equal(
		textUsage.trim(),
		"Invalid command 'bogus'; valid commands: send, crew init, member status, session list, member follow-up, member redirect, member inbox send, crew broadcast",
	);
...
test("PO: --session is not a global/root flag today; it must be added as an explicit contract decision", () => {
	...
	assert.throws(
		...
		/Invalid command '--session'; valid commands: send, crew init, member status, session list, member follow-up, member redirect, member inbox send, crew broadcast/,
	);
});
