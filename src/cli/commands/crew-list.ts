...
export async function runCrewListCommand(
	...
	for (const candidate of candidates) {
		try {
			const manifest = await deps.readManifest(candidate.manifestPath, projectRoot);
			const probed = await probeCrew(manifest, deps, context.signal);
			rows.push({
	...
	const observed = await deps.readObservedLocators(projectRoot);
	const records: Array<{
		manifestPath: string;
		observedAt: string;
		lastSeenAt?: string;
	...
	for (const record of records) {
		...
		try {
			const manifest = await deps.readManifest(record.manifestPath, projectRoot);
