...
import {
	...
	MAX_CREW_UPDATE_BUFFERED,
	MAX_CREW_UPDATE_INBOUND,
	MAX_CREW_UPDATE_OUTBOUND,
	MAX_CREW_UPDATE_REQUEST_ID_BYTES,
	MAX_CREW_UPDATE_TOMBSTONES,
} from "./crew-update.ts";

const member = { name: "qa", role: "reviewer" };
const request = (registry: CrewUpdateRegistry, requestId: string, now = 1_000) =>
	registry.registerOutbound({ requestId, member, now });
function arm(registry: CrewUpdateRegistry, requestId: string): void {
...
test("registers before acceptance, starts the default deadline immediately, and cleans pre-accept failures", () => {
	const registry = new CrewUpdateRegistry();
	const result = request(registry, "request-1");
	assert.equal(result.ok, true);
	if (result.ok) assert.equal(result.value.deadlineAt, 1_000 + DEFAULT_CREW_UPDATE_TIMEOUT_SECONDS * 1_000);
	...
	for (const timeoutSeconds of [0, -1, 601, 1.5, Number.NaN]) {
		const invalid = new CrewUpdateRegistry();
		assert.deepEqual(
...
test("enforces outbound/inbound capacities and the UTF-8 request id bound before mutation", () => {
	const outbound = new CrewUpdateRegistry();
	for (let index = 0; index < MAX_CREW_UPDATE_OUTBOUND; index += 1)
	...
	assert.deepEqual(request(new CrewUpdateRegistry(), "😀".repeat(MAX_CREW_UPDATE_REQUEST_ID_BYTES)), {
		ok: false,
		code: "invalid-request-id",
	});
	const inbound = new CrewUpdateRegistry();
	for (let index = 0; index < MAX_CREW_UPDATE_INBOUND; index += 1)
		assert.equal(
...
test("response wins when resolved first, while idle closes and late response expires", () => {
	const responseFirst = new CrewUpdateRegistry();
	...
	const idleFirst = new CrewUpdateRegistry();
...
test("tombstones are bounded while the newest idle late-response recovery remains available", () => {
	const registry = new CrewUpdateRegistry();
	for (let index = 0; index < MAX_CREW_UPDATE_TOMBSTONES + 1; index += 1) {
	...
	assert.deepEqual(
		registry.resolveResponse({
			requestId: `tombstone-${MAX_CREW_UPDATE_TOMBSTONES}`,
			member,
			message: "late",
			instructions: [],
		}),
...
test("buffer capacity is reserved by active requests and rejects before delivery", () => {
	const registry = new CrewUpdateRegistry();
	for (let index = 0; index < MAX_CREW_UPDATE_BUFFERED; index += 1) {
		const id = `buffer-${index}`;
		assert.equal(request(registry, id).ok, true);
		assert.equal(registry.resolveTimeout(id).ok, true);
	}
	assert.equal(registry.bufferedCount(), MAX_CREW_UPDATE_BUFFERED);
	...
	assert.equal(registry.bufferedCount(), MAX_CREW_UPDATE_BUFFERED - 1);
});
