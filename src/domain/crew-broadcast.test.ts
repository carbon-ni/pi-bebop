...
describe("Crew Broadcast domain", () => {
	test("validates message and ordered instructions without accepting persistence fields", () => {
		assert.doesNotThrow(() => validateBroadcastInput({ senderName: "Bob", content: "hello", instructions: ["one"] }));
		assert.throws(() => validateBroadcastInput({ senderName: "Bob", content: " " }), /non-empty message/);
		assert.throws(() => validateBroadcastInput({ senderName: "Bob", content: "hello", instructions: [""] }), /instructions/);
	});
	...
	if (!result.ok) return;
	assert.deepEqual(result.recipients.map((recipient) => recipient.member.name), ["Tony", "Mary", "Kelly"]);
