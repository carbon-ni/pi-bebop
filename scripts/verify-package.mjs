...
try {
	...
	let cliError;
	try {
		await execFile(process.execPath, [cli, "member", "status", "--bogus"], {
			cwd: consumerDir,
			env: environment,
		});
	} catch (error) {
	...
	if (cliError?.code !== 2 || !/unknown option '--bogus'/.test(cliError.stderr ?? ""))
		throw new Error("Installed CLI verification failed");

	// Packed CLI must ship the deterministic crew init templates and
	...
	try {
		...
		// Canonical leaf help is generated by Commander; verify its command and
		// declared options without byte snapshots.
		const statusHelp = await execFile(process.execPath, [cli, "member", "status", "--help"], {
			cwd: initDir,
			env: environment,
		});
		if (
			...
			throw new Error("Installed CLI member status --help missing metadata defaults");
		...
		// Execute the generated bin shim/symlink directly (not only
		...
		if (
			...
			!/-h, --help/.test(binRootHelp.stdout) ||
